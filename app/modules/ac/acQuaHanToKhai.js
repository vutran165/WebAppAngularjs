acModule.factory('acQuaHanToKhaiService', [
    '$resource', '$http', '$window', 'configService', 'clientService',
    function ($resource, $http, $window, configService, clientService) {
        var rootUrl = configService.apiServiceBaseUri;
        var serviceUrl = rootUrl + '/api/Ac/Output';
        var calc = {
        ...
        var result = {
            ...
            postReport: function (data, callback) {
                $http.post(serviceUrl + '/PostReportQHTK', data).success(callback);
            },
            postExportExcel: function (json, filename) {
                $http({
                    url: serviceUrl + '/PostExportExcelQHTK',
                    method: "POST",
...
acModule.controller('acQuaHanToKhaiController', [
    '$scope', '$rootScope', '$window', '$stateParams', '$timeout', '$sce', '$state', '$uibModal', '$log',
    'mdService', 'acQuaHanToKhaiService', 'clientService', 'configService', '$filter', 'serviceInventoryAndWareHouse', 'serviceInventoryAndMerchandiseType', 'serviceInventoryAndMerchandise', 'serviceInventoryAndMerchandiseGroup', 'serviceInventoryAndCompany', 'serviceInventoryAndDonViHaiQuan', 'serviceInventoryAndUnitUser',
    function ($scope, $rootScope, $window, $stateParams, $timeout, $sce, $state, $uibModal, $log,
        ...
        function filterData() {
            ...
            acQuaHanToKhaiService.postQuery(
            ...
            acQuaHanToKhaiService.getNewParameter(function (response) {
        ...
        $scope.report = function () {
            ...
            $state.go('acQuaHanToKhaiReport', { obj: $scope.target });
        }
        $scope.printExcel = function () {
            acQuaHanToKhaiService.postExportExcel($scope.target, "BaoCaoQuaHanToKhai");
        }
    }
]);
nvModule.controller('reportQuaHanToKhaiController', ['$scope', '$window', '$stateParams', '$timeout', '$state',
    'nvService', 'acQuaHanToKhaiService', 'mdService', 'clientService',
function ($scope, $window, $stateParams, $timeout, $state,
nvService, acQuaHanToKhaiService, mdService, clientService) {
    var para = $state.params.obj;
    $scope.robot = angular.copy(acQuaHanToKhaiService.robot);
    $scope.tempData = mdService.tempData;
    $scope.target = [];
    $scope.goIndex = function () {
        $state.go('acQuaHanToKhai');
    }
    function filterData() {
        if (para) {
            acQuaHanToKhaiService.postReport(para, function (response) {


                $scope.target = response;
                $scope.target.fromDate = para.fromDate;
                $scope.target.toDate = para.toDate;
                console.log('aaa', $scope.target);
            });
        }
    };
