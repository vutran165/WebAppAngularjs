acModule.factory('xuatBanKHNoiThanhService', [
    ...
    function ($resource, $http, $window, configService, clientService) {
        ...
        var result = {
            ...
            postReportXuatBanKHNoiThanh: function (data, callback) {
                $http.post(serviceUrl + '/PostReportXuatBanKHNoiThanh', data).success(callback);
            },
            postExportExcelXuatBanKH: function (json, filename) {
                $http({
                    url: serviceUrl + '/PostExportExcelXuatBanKHNoiThanh',
                    method: "POST",
...
acModule.controller('xuatBanKHNoiThanhController', ['$scope', '$rootScope', '$window', '$stateParams', '$timeout', '$sce', '$state', '$uibModal', '$log',
'mdService', 'xuatBanKHNoiThanhService', 'clientService', 'configService', '$filter', 'serviceInventoryAndMerchandiseGroup', 'serviceInventoryAndWareHouse', 'serviceInventoryAndMerchandiseType', 'serviceInventoryAndMerchandise', 'inventoryService', 'serviceInventoryAndCompany', 'serviceInventoryAndDonViHaiQuan', 'serviceInventoryAndUnitUser', 'serviceInventoryAndDoiTuong',
...
mdService, xuatBanKHNoiThanhService, clientService, configService, $filter, serviceInventoryAndMerchandiseGroup, serviceInventoryAndWareHouse, serviceInventoryAndMerchandiseType, serviceInventoryAndMerchandise, inventoryService, serviceInventoryAndCompany, serviceInventoryAndDonViHaiQuan, serviceInventoryAndUnitUser, serviceInventoryAndDoiTuong) {
    ...
    function filterData() {
        ...
        xuatBanKHNoiThanhService.getNewParameter(function (response) {
            $scope.target = response;
        });
        xuatBanKHNoiThanhService.postQuery(
    ...
    $scope.report = function () {
        ...
        $state.go('reportXuatBanKHNoiThanh', { obj: $scope.target });

    }
    $scope.exportToExcel = function () {
        xuatBanKHNoiThanhService.postExportExcelXuatBanKH($scope.target, "BaoCaoXuatBanKhachHangNoiThanh");
    }
...
nvModule.controller('reportXuatBanKHNoiThanhController', ['$scope', '$window', '$stateParams', '$timeout', '$state',
    'nvService', 'xuatBanKHNoiThanhService', 'mdService', 'clientService',
function ($scope, $window, $stateParams, $timeout, $state,
nvService, xuatBanKHNoiThanhService, mdService, clientService) {
    var para = $state.params.obj;
    $scope.robot = angular.copy(xuatBanKHNoiThanhService.robot);
    $scope.tempData = mdService.tempData;
    $scope.target = [];
    $scope.goIndex = function () {
        $state.go('xuatBanKHNoiThanh');
    }
    $scope.target.data = [];
    function filterData() {
        ...
        if (para) {
            xuatBanKHNoiThanhService.postReportXuatBanKHNoiThanh(para, function (response) {
