var cuaHangNhapTuNguonTrongNuocService = nvModule.factory('cuaHangNhapTuNguonTrongNuocService',
[
    '$resource', '$http', '$window', 'configService', 'clientService',
    function ($resource, $http, $window, configService, clientService) {
        var rootUrl = configService.apiServiceBaseUri;
        var serviceUrl = rootUrl + '/api/Nv/TiepNhanHangNhapCh/NhapTuNguonTrongNuoc';
...
var cuaHangNhapTuNguonTrongNuocController = nvModule.controller('cuaHangNhapTuNguonTrongNuocController', [
    '$scope', '$rootScope', '$location', '$window', '$uibModal', '$log', '$state', '$filter', '$http',
    'cuaHangNhapTuNguonTrongNuocService', 'configService', 'clientService', 'nvService', 'mdService', 'blockUI', 'serviceNhapHangAndMerchandise', '$mdDialog', 'authorizeService', 'serviceCompanySelected',
    function (
        $scope, $rootScope, $location, $window, $uibModal, $log, $state, $filter, $http,
        cuaHangNhapTuNguonTrongNuocService, configService, clientService, nvService, mdService, blockUI, serviceNhapHangAndMerchandise, $mdDialog, authorizeService, serviceCompanySelected
    ) {
        ...
        $scope.selectedCompany = function (code) {
            if (code) {
                cuaHangNhapTuNguonTrongNuocService.getSelectCompanyByCode(code).then(function (response) {
        ...
        function filterData() {
            ...
            cuaHangNhapTuNguonTrongNuocService.postQuery(
        ...
        $scope.title = function () {
            return 'Kê khai hàng hóa nhập trực tiếp vào cửa hàng từ nguồn hàng sản xuất trong nước';
        };
        ...
        $scope.create = function () {
            ...
            var modalInstance = $uibModal.open({
                backdrop: 'static',
                templateUrl: nvService.buildUrlTax('TiepNhanHangNhapCuaHang', 'NhapTuNguonTrongNuoc', 'add'),
                controller: 'cuaHangNhapTuNguonTrongNuocCreateController',
        ...
        $scope.details = function (target) {
            var modalInstance = $uibModal.open({
                backdrop: 'static',
                templateUrl: nvService.buildUrlTax('TiepNhanHangNhapCuaHang', 'NhapTuNguonTrongNuoc', 'details'),
                controller: 'cuaHangNhapTuNguonTrongNuocDetailsController',
                windowClass: 'app-modal-window',
                //size: 'lg',
                resolve: {
                    targetData: function () {
        ...
        $scope.histories = function (target) {
            var modalInstance = $uibModal.open({
                backdrop: 'static',
                templateUrl: nvService.buildUrlTax('TiepNhanHangNhapCuaHang', 'NhapTuNguonTrongNuoc', 'histories'),
                controller: 'cuaHangNhapTuNguonTrongNuocHistoryController',
                windowClass: 'app-modal-window',
                //size: 'lg',
                resolve: {
                    targetData: function () {
        ...
        $scope.printITem = function (target) {
            var modalInstance = $uibModal.open({
                backdrop: 'static',
                templateUrl: nvService.buildUrlTax('TiepNhanHangNhapCuaHang', 'NhapTuNguonTrongNuoc', 'printItem'),
                controller: 'cuaHangNhapTuNguonTrongNuocExportItemController',
                windowClass: 'app-modal-window',
                resolve: {
                    targetData: function () {
                        return target;
        ...
        $scope.printPopup = function (target) {
            var modalInstance = $uibModal.open({
                backdrop: 'static',
                templateUrl: nvService.buildUrlTax('TiepNhanHangNhapCuaHang', 'NhapTuNguonTrongNuoc', 'report'),
        ...
        $scope.update = function (target) {
            var modalInstance = $uibModal.open({
                backdrop: 'static',
                templateUrl: nvService.buildUrlTax('TiepNhanHangNhapCuaHang', 'NhapTuNguonTrongNuoc', 'update'),
                controller: 'cuaHangNhapTuNguonTrongNuocEditController',
                windowClass: 'app-modal-window',
                //size:'lg',
                resolve: {
                    targetData: function () {
        ...
        $scope.print = function () {
            ...
            cuaHangNhapTuNguonTrongNuocService.setParameterPrint(
                postdata);
            $state.go("nvPrintcuaHangNhapTuNguonTrongNuoc");
        }
        $scope.printDetail = function () {
            ...
            cuaHangNhapTuNguonTrongNuocService.setParameterPrint(
                postdata);
            $state.go("nvPrintDetailcuaHangNhapTuNguonTrongNuoc");
        }
        ...
        $scope.deleteItem = function (ev, item) {
            ...
            $mdDialog.show(confirm).then(function () {
                cuaHangNhapTuNguonTrongNuocService.deleteItem(item).then(function (data) {
                    console.log(data);
                }).then(function (data) {
                    $mdDialog.show(
                            $mdDialog.alert()
        ...
        $scope.import = function () {
            var modalInstance = $uibModal.open({
                backdrop: 'static',
                templateUrl: nvService.buildUrlTax('TiepNhanHangNhapCuaHang', 'NhapTuNguonTrongNuoc', 'import'),
                controller: 'cuaHangNhapTuNguonTrongNuocImportController',
                size: 'md'
            });
            modalInstance.result.then(function (updatedData) {
                serviceNhapHangAndMerchandise.getSelectData().clear();
...
nvModule.controller('cuaHangNhapTuNguonTrongNuocDetailsController', [
'$scope', '$uibModalInstance',
'mdService', 'cuaHangNhapTuNguonTrongNuocService', 'targetData', 'clientService', 'configService', '$filter',
function ($scope, $uibModalInstance,
    ...
    $scope.approval = function () {
        cuaHangNhapTuNguonTrongNuocService.postApproval($scope.target, function (response) {
            if (response) {

                alert("Duyệt thành công!");
                $uibModalInstance.close($scope.target);
                $scope.goIndex = function () {
                    $state.go('nvCuaHangNhapTuNguonTrongNuoc');
                };
    ...
    function fillterData() {
        $scope.isLoading = true;
        cuaHangNhapTuNguonTrongNuocService.getDetails($scope.target.id, function (response) {
            console.log(response);
            if (response.status) {
...
nvModule.controller('cuaHangNhapTuNguonTrongNuocHistoryController', [
'$scope', '$uibModalInstance',
'mdService', 'cuaHangNhapTuNguonTrongNuocService', 'targetData', 'clientService', 'configService', '$filter',
function ($scope, $uibModalInstance,
    mdService, cuaHangNhapTuNguonTrongNuocService, targetData, clientService, configService, $filter) {
    ...
    $scope.tempData = mdService.tempData;
    $scope.title = function () {
        return 'Phiếu kê khai hàng hóa nhập trực tiếp vào cửa hàng từ nguồn hàng sản xuất trong nước ';
    };
    ...
    function fillterData() {
        ...
        cuaHangNhapTuNguonTrongNuocService.getHistories(choiceObj, function (response) {
            if (response.status) {
...
nvModule.controller('cuaHangNhapTuNguonTrongNuocCreateController', [
    '$scope', '$rootScope', '$uibModalInstance', 'clientService', '$filter', '$state', '$uibModal', '$log',
    'nvService', 'cuaHangNhapTuNguonTrongNuocService', 'mdService', 'configService', 'serviceNhapHangAndMerchandise', 'focus',
    function ($scope, $rootScope, $uibModalInstance, clientService, $filter, $state, $uibModal, $log,
        nvService, cuaHangNhapTuNguonTrongNuocService, mdService, configService, serviceNhapHangAndMerchandise, focus) {
        $scope.robot = angular.copy(cuaHangNhapTuNguonTrongNuocService.robot);
        $scope.paged = angular.copy(configService.pageDefault);
        $scope.config = nvService.config;
        $scope.tempData = mdService.tempData;
        $scope.newItem = {};
                ...
                if (exsist) {
                    ...
                    angular.forEach($scope.target.dataDetails, function (v, k) {
                        if (v.maHang == $scope.newItem.maHang && $scope.newItem.soToKhai == v.soToKhai) {
                            ...
                            cuaHangNhapTuNguonTrongNuocService.robot.changeSoLuong($scope.target.dataDetails[k]);
                        }
                    });
                } else {
        ...
        $scope.selectedMaHang = function (code) {
            if (code) {
                cuaHangNhapTuNguonTrongNuocService.getMerchandiseForNvByCode(code).then(function (response) {
        ...
        //    cuaHangNhapTuNguonTrongNuocService.getWareHouse(item.id, function (response) {
        ...
        $scope.save = function () {
            $scope.Loading = true;
            cuaHangNhapTuNguonTrongNuocService.post(
        ...
        function filterData() {
            $scope.isLoading = true;
            cuaHangNhapTuNguonTrongNuocService.getNewInstance(function (response) {
...
nvModule.controller('cuaHangNhapTuNguonTrongNuocEditController', [
    '$scope', '$uibModalInstance', 'clientService', '$filter', '$state', '$uibModal', '$log',
    'nvService', 'cuaHangNhapTuNguonTrongNuocService', 'mdService', 'targetData', 'configService', 'serviceNhapHangAndMerchandise', 'focus',
    function ($scope, $uibModalInstance, clientService, $filter, $state, $uibModal, $log,
        ...
        $scope.save = function () {
            cuaHangNhapTuNguonTrongNuocService.updateCT($scope.target).then(
                    function (response) {
            ...
            cuaHangNhapTuNguonTrongNuocService.updateCT($scope.target).then(
                    function (response) {
                        if (response.data.status) {
                            ...
                            var url = $state.href('reportcuaHangNhapTuNguonTrongNuoc', { id: response.data.data.id });
...
nvModule.controller('reportCuaHangNhapTuNguonTrongNuocController', [
    '$scope','$rootScope' ,'$filter', '$window', '$stateParams', '$timeout', '$state', '$uibModalInstance', 'targetData',
    'mdService', 'cuaHangNhapTuNguonTrongNuocService', 'clientService',
    function ($scope, $rootScope, $filter, $window, $stateParams, $timeout, $state, $uibModalInstance, targetData,
        mdService, cuaHangNhapTuNguonTrongNuocService, clientService) {
        $scope.robot = angular.copy(cuaHangNhapTuNguonTrongNuocService.robot);
        ...
        $scope.title = "PHIẾU KÊ KHAI HÀNG HÓA NHẬP TRỰC TIẾP CỬA HÀNG TỪ NGUỒN HÀNG SX TRONG NƯỚC";

        function filterData() {
            if (targetData.id) {
                cuaHangNhapTuNguonTrongNuocService.getReport(targetData.id, function (response) {
                    console.log(response);
                    if (response.status) {
                        $scope.target = response.data;
...
nvModule.controller('printcuaHangNhapTuNguonTrongNuocController', [
    ...
    function ($scope, $state, $window, $stateParams, $timeout, $filter,
        ...
        function filterData() {
            cuaHangNhapTuNguonTrongNuocService.postPrint(
                function (response) {
                    $scope.printData = response;
                });
        };

        $scope.info = cuaHangNhapTuNguonTrongNuocService.getParameterPrint().filtered.advanceData;
        $scope.goIndex = function () {
            $state.go("nvCuaHangNhapTuNguonTrongNuoc");
        }
        $scope.printExcel = function () {
...
nvModule.controller('printDetailcuaHangNhapTuNguonTrongNuocController', [
    '$scope', '$state', '$window', '$stateParams', '$timeout', '$filter',
    'mdService', 'cuaHangNhapTuNguonTrongNuocService', 'clientService',
    function ($scope, $state, $window, $stateParams, $timeout, $filter,
        mdService, cuaHangNhapTuNguonTrongNuocService, clientService) {
        ...
        function filterData() {
            cuaHangNhapTuNguonTrongNuocService.postPrintDetail(
                function (response) {
                    $scope.printData = response;
                });
        }
...
nvModule.controller('cuaHangNhapTuNguonTrongNuocExportItemController', [
    '$scope', '$filter', '$window', '$stateParams', '$timeout', '$state',
    'mdService', 'cuaHangNhapTuNguonTrongNuocService', 'clientService', '$uibModalInstance', 'targetData', 'Excel', 'configService', '$location',
    function ($scope, $filter, $window, $stateParams, $timeout, $state,
        ...
        cuaHangNhapTuNguonTrongNuocService.getUnitName($scope.maDonVi).then(function (response) {
            $scope.tenDonVi = response.data;
        });
        cuaHangNhapTuNguonTrongNuocService.getInfoItemDetails($scope.target.id, function (response) {
            console.log(response);
            if (response.status) {
        ...
        $scope.exportToExcel = function () {
            cuaHangNhapTuNguonTrongNuocService.writeDataToExcel($scope.dataHangHoa, function (response) {
                if (response.status) {
                    clientService.noticeAlert("Thành công", "success");
                    //  $uibModalInstance.close($scope.target);
                }
